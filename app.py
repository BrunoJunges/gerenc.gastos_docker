...
# --- Configurações ---
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'default_dev_secret_key_for_local_testing')
...
def get_db_connection():
    # CORREÇÃO: Aponta para o banco de dados na pasta 'instance'
    db_path = os.path.join(app.instance_path, 'finance.db')
    os.makedirs(app.instance_path, exist_ok=True) # Garante que a pasta instance exista
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    return conn

def obter_valor_convertido(valor_original, moeda_original):
    if moeda_original == 'BRL': return valor_original
...
def atualizar_gasto(id):
    ...
    updated_gasto = updated_gasto_conn.execute('SELECT g.*, c.nome as categoria_nome FROM gastos g JOIN categorias c ON g.categoria_id = c.id WHERE g.id = ?', (id,)).fetchone()
...
def get_summary():
    # ... (código do summary que você já tem)
