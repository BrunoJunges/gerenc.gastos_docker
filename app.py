...
def criar_gasto():
    ...
    user_id = session['user_id']
    conn = get_db_connection()
    categoria = conn.execute('SELECT id, nome FROM categorias WHERE id = ? AND user_id = ?', (data['categoria_id'], user_id)).fetchone()
    if not categoria:
        conn.close()
    ...
    new_gasto = new_gasto_conn.execute('SELECT g.id, g.descricao, g.data, g.valor_original, g.moeda_original, g.valor_brl, g.info_parcela, c.nome as categoria_nome FROM gastos g JOIN categorias c ON g.categoria_id = c.id WHERE g.id = ?', (new_id,)).fetchone()
...
def get_summary():
    ...
    categorias_usuario = conn.execute('SELECT nome FROM categorias WHERE user_id = ? ORDER BY nome', (user_id,)).fetchall()
    ...
    categoria_abreviacoes_db = {
        "Alimentação": "Alim", "Locomoção": "Locom", "Casa": "Casa", "Lazer": "Lazer", "Despesas inesperadas": "Desp.In",
        "Parcelas/Crédito": "Parc.", "Outros/Indefinível": "Outro"
    }

    for cat_nome, total_cat in gastos_por_categoria.items():
        percentual = (total_cat / total_gasto) * 100 if total_gasto > 0 else 0
        abrev = categoria_abreviacoes_db.get(cat_nome, cat_nome[:4])
        grafico_data.append({
    ...
    conn.close()
    return jsonify({'total_gasto': total_gasto, 'grafico_data': grafico_data})
